...
MeasureOmni3W::MeasureOmni3W(const W_PARAM &_w_param) : w_param(_w_param) {}

MoveOmni3W::MoveOmni3W(const W_PARAM &_w_param) : w_param(_w_param) {}

void MeasureOmni3W::cal_disp(const float encoder[], size_t mysize,
                             ODOM &dist_r) {
  if (mysize != 3) {
...
void MoveOmni3W::cal_cmd(const ODOM &cmd_r, float wheel_cmd[], size_t mysize) {
  if (mysize != 3) {
    std::printf("invalid argument size");
  } else {
