...
void illias::MeasureSteering::cal_disp(std::shared_ptr<float[]> encoder,
  ...
  if (length != 8) {
    ...
    delta.theta =
        this->rotate_to_meter(encoder[0]) * sin(encoder[4] - table_arg) -
    ...
    delta.theta /=
        (4 * sqrt(pow(w_param.length_x / 2, 2) + pow(w_param.length_y / 2, 2)));
  }
...
void illias::MoveSteering::cal_cmd(const CMD &cmd, const float &table_angle) {
  ...
  for (int i = 0; i < 8; i++) {
    printf("wheel_cmd_meter[%d] = %f\n", i, wheel_cmd_meter[i]);
    if (i < 4) {
      wheel_cmd_rotate[i] = meter_to_rotate(wheel_cmd_meter[i]);
    } else {
      wheel_cmd_rotate[i] = rad_to_rotate(
          wheel_cmd_meter[i] * w_param.gear_ratio_horizonal - table_angle);
    }
  }
}