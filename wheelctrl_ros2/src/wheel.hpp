...
//   // convert encoder data into robot posture(world coordinate)
//   virtual void cal_disp(std::shared_ptr<float[]> encoder,const int
//   &length,const float &imu){//エンコーダーの値から現在の座標を計算
//     printf("please use subclass");
//     for (int i = 0; i < length;i++){
//       printf("%f ", encoder[i]);
//     }
...
//     return (rotate/this->w_param.gear_ratio) * 2 * M_PI *
//     this->w_param.radius;
//   }
...
//   }
//   virtual void cal_cmd(const CMD &cmd, const float &table_angle,const float
//   curvature){
//     printf("please use subclass");
...
//     return meter * this->w_param.gear_ratio / (2 * M_PI *
//     this->w_param.radius);
//   }
